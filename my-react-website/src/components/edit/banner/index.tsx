...
function EditBanner() {
  ...
  const getBannerData = async () => {
    ...
    try {
      ...
      const bannerData = bannerResponse.data[0]; // เลือกแบนเนอร์

      // set state
      setFormData({
        banner: bannerData.link,
      });
    } catch (error: any) {
  ...
  const handleSave = async () => {
    ...
    const token = localStorage.getItem("token");
    try {
      // ทำการบันทึกข้อมูลแบนเนอร์
      await axios.put(
        "http://localhost:3001/api/banner",
        {
          link: formData.banner,
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    ...
    >
      ...
      <Dialog open={openCancel} onClose={() => setOpenCancel(false)}>
        <DialogTitle>ยกเลิกการ Edit Banner</DialogTitle>
        <DialogContent>
          คุณต้องการยกเลิกการ Edit Banner ใช่หรือไม่ ?
        </DialogContent>
        <DialogActions>
          <Button
            style={{ marginRight: "10px" }}
      ...
      <Dialog open={openAdd} onClose={() => setOpenAdd(false)}>
        <DialogTitle>Edit Banner Success</DialogTitle>
        <DialogContent>Edit Banner สำเร็จเเล้ว ! </DialogContent>
        <DialogActions>
          <Button
            style={{ marginRight: "10px" }}
            variant="contained"
      ...
      <Dialog open={openAddFail} onClose={() => setOpenAddFail(false)}>
        <DialogTitle>Edit Banner Fail</DialogTitle>
        <DialogContent>
          Edit Banner ไม่สำเร็จ กรุณาตรวจสอบความถูกต้องอีกครั้ง{" "}
        </DialogContent>
        <DialogActions>
          <Button
            style={{ marginRight: "10px" }}
